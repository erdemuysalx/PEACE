...
UAVTools executes the steps deterministically with no LLM in the execution loop.
"""
...
from peace.backends.llm import OllamaClient
from peace.core.config import get_settings
from peace.exceptions import InferenceError
from peace.platforms import UAVTools
from peace.system_prompts import LANGUAGE_SYSTEM_PROMPT, format_world_context
from peace.schemas import RobotState, ExecutionStatus, MissionPlan, ToolCall, WorldModel
from peace.services.mission_logger_service import MissionLoggerService
from peace.services.robot_state_service import RobotStateAggregatorService
from peace.services.constraint_enforcement_service import ConstraintEnforcementService
...
class AgentNode(Node):
    ...
    single inference call. UAVTools then executes each step deterministically
    ...
    def __init__(self) -> None:
        ...
        self.constraint_svc = ConstraintEnforcementService(
        ...
        self.mission_logger = MissionLoggerService(
        ...
        # ── UAVTools (owns MAVROS publisher + service clients) ──────────────
        self.tools = UAVTools(
            self, self.ego_state_svc, self.world_model_svc, self.constraint_svc, self.settings
        )
    ...
    def destroy_node(self) -> None:
        ...
        self.mission_logger.close()
        super().destroy_node()

    # ── Telemetry callbacks ───────────────────────────────────────────────────

    def _state_cb(self, msg: State) -> None:
        """Forward MAVROS State to UAVTools and RobotStateAggregatorService."""
    ...
    def _process_query(
        ...
        model_name = self.settings.language_model
        try:
            ...
            step_summaries = [
                ...
                if s.args else s.tool
                for s in plan.steps
            ]
            self._publish_execution_status(
                mission_id, "executing", "Executing plan",
                reasoning=plan.reasoning, steps=step_summaries,
                current_index=0, total=len(plan.steps),
            )
            with self._lock:
        ...
        try:
            for i, step in enumerate(steps):
                self._publish_execution_status(
                    ...
                    current_index=i + 1, total=len(steps),
                )
            ...
            # Only the terminal call in the replan recursion writes the row, so each
            # mission produces exactly one log entry. replan_depth doubles as the
            # replan_count for that mission.
            if final_state != "replanning":
                final_ego = self.ego_state_svc.get_ego_state()
                self.mission_logger.log_mission(
                    ...
                    replan_count=replan_depth, model=model_name,
                )
                with self._lock:
                    self._executing = False
                self.tools.stop_safely()
